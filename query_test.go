...
func init() {
	...
	mockNs = NewMockKeySpace()
}

func TestEq(t *testing.T) {
	test(func(n KeySpace) {
		cs := n.Table("customer", Customer{}, Keys{PartitionKeys: []string{"Id"}})
...
func TestIn(t *testing.T) {
	test(func(k KeySpace) {
		cs := k.Table("customer", Customer{}, Keys{
...
func TestAnd(t *testing.T) {
	test(func(k KeySpace) {
		cs := k.Table("customer1", Customer{}, Keys{PartitionKeys: []string{"Id", "Name"}})
		createIf(cs.(TableChanger), t)
		err := cs.Set(Customer{
			Id:   "100",
			Name: "Joe",
...
func TestQueryReturnError(t *testing.T) {
	test(func(k KeySpace) {
		cs := k.Table("customer2", Customer{}, Keys{})
		res := []Customer{}
		err := cs.Where(Eq("Id", "100"), Eq("Name", "Joe")).Read(&res).Run()
		if err == nil {
...
func TestRowNotFoundError(t *testing.T) {
	test(func(k KeySpace) {
		cs := k.MapTable("customer", "Id", Customer{})
		createIf(cs.(TableChanger), t)
		c := &Customer{}
...
func TestTypesMarshal(t *testing.T) {
	test(func(k KeySpace) {
		c := newCustomer3()
		tbl := k.Table("customer3", Customer3{}, Keys{PartitionKeys: []string{"Id"}})
...
func TestUpdateList(t *testing.T) {
	test(func(k KeySpace) {
		tbl := k.Table("customer34", Customer3{}, Keys{PartitionKeys: []string{"Id"}})
		createIf(tbl.(TableChanger), t)
		c := newCustomer3()
		if err := tbl.Set(c).Run(); err != nil {
			t.Fatal(err)
...
func TestCQLInjection(t *testing.T) {
	test(func(k KeySpace) {
		tbl := k.Table("customer45", Customer3{}, Keys{PartitionKeys: []string{"Id"}})
		createIf(tbl.(TableChanger), t)
		c := Customer3{
			Id:      "1",
			Strings: []string{"a", "b", "c"},
...
func TestMaps(t *testing.T) {
	test(func(k KeySpace) {
		tbl := k.MapTable("customer34213", "Id", CustomerWithMap{})
		createIf(tbl.(TableChanger), t)
		c := CustomerWithMap{
			Id: "1",
			Map: map[string]string{
...
func TestCounters(t *testing.T) {
	test(func(k KeySpace) {
		tbl := k.MapTable("customer4985", "Id", CustomerWithCounter{})
		createIf(tbl.(TableChanger), t)
...
func TestNoop(t *testing.T) {
	test(func(k KeySpace) {
		...
		tbl := k.MapTable("customer4987", "Id", CustomerWithCounter{})
		createIf(tbl.(TableChanger), t)
		c := CustomerWithCounter{
			Id:      "1",
			Counter: Counter(0),
